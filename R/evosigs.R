...
evoSigs <- function( survivalData, evolutionarySteps ) {
    ...
    if (isValid == TRUE) {
        ...
        survivalAnalysis$significantCoefficients <- significantCoefficients
        ...
        if (length(range) == 2) {
            ...
            survivalAnalysis$clusters[survivalAnalysis$beta >= range[2]] <- "C3"
        }
        if (length(range) == 3) {
            survivalAnalysis$clusters[survivalAnalysis$beta >= range[1]] <- "C2"
            survivalAnalysis$clusters[survivalAnalysis$beta >= range[2]] <- "C3"
            survivalAnalysis$clusters[survivalAnalysis$beta >= range[3]] <- "C4"
        }
        clusters <- rep(NA, length(beta))
        clustPos <- 0
        for (currClustPos in sort(unique(survivalAnalysis$clusters))) {
            clustPos <- clustPos + 1
            clusters[which(survivalAnalysis$clusters == currClustPos)] <- paste0("C", clustPos)
        }
    ...
    if (isValid == TRUE) {
        ...
        for (i in rownames(freqFeatures)) {
            currSamplesData <- evolutionarySteps[which(clusters==i), , drop=FALSE]
